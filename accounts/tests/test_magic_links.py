...
def test_issued_token_can_be_consumed_once():
    token = magic_links.issue_token(account_id=42, channel="email", destination="a@example.com")

    payload = magic_links.consume_token(token)
    assert payload == {"account_id": 42, "channel": "email", "destination": "a@example.com"}
...
def test_rate_limit_allows_up_to_the_max_then_blocks():
    ...
    account_id = random.randint(1_000_000, 2_000_000)

    results = [magic_links.is_rate_limited(account_id) for _ in range(magic_links.RATE_LIMIT_MAX_PER_WINDOW)]
    assert all(blocked is False for blocked in results)

    assert magic_links.is_rate_limited(account_id) is True
